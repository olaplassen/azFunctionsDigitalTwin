...
namespace SampleFunctionsApp
{
    ...
    public static class ProcessDTRoutedData
    {
        ...
        {
            log.LogInformation("Start execution ProcessDTRoutedData");
            ...
            if (client != null)
            {
                if (eventGridEvent != null && eventGridEvent.Data != null)
                {
                    ...
                    string parentId = await AdtUtilities.FindParentAsync(client, twinId, "sensors", log);
                    log.LogInformation($"parentId: {parentId}");
                    
                    if (parentId != null)
                    {
                        log.LogInformation($"Found parent id: {parentId}");
                        ...
                        foreach (var operation in message["data"]["patch"])
                        {
                            ...
                            if (opValue.Equals("replace"))
                            {
                                ...
                                {
                                    log.LogInformation($"Updating parent twin temperature..");
                                ...
                                {
                                    log.LogInformation($"Updating parent twin Humidity..");
                                    await AdtUtilities.UpdateTwinPropertyAsync(client, parentId, propertyPath, operation["value"].Value<float>(), log);
                                }
                                else
                                {
                                    log.LogInformation($"Not coorect path: {propertyPath}");
                                }
                               
                            }
                            else
                            {
                                log.LogInformation($"No correct operation value");
                            }
                        }
                    }
                    else
                    {
                        log.LogInformation($"Parent id == null");
                    }
                }
            }
        }
